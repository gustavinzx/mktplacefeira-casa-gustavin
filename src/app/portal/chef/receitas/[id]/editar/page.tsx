...
export default function EditarReceitaPage({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (rec.ingredients && rec.ingredients.length > 0) {
            // Remove the auto-extracted ingredients before editing, so they are not saved as manual
            const manualIngredients = rec.ingredients.filter((ing: any) => !ing.is_auto_extracted);
            if (manualIngredients.length > 0) {
              setIngredients(manualIngredients.map((i: any) => ({ name: i.name, amount: i.amount || '' })));
            }
          }
        } else {
          setError(data.error || 'Erro ao carregar a receita');
        }
      } catch (err) {
        setError('Erro de conexão ao buscar a receita.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error || 'Erro ao atualizar receita');
      }
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 800 }}>
      <header style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontSize: 28, color: '#333' }}>Editar Receita</h1>
        <p style={{ color: '#666' }}>Atualize sua criação.</p>
      </header>
      ...
      <form onSubmit={handleSubmit} style={{ background: '#fff', padding: 32, borderRadius: 12, border: '1px solid #eee' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <label style={{ fontWeight: 600, color: '#333' }}>Ingredientes Manuais</label>
            <button 
              type="button" 
        ...
        >
          {loading ? <Loader2 className="animate-spin" /> : <><Save size={20} /> Atualizar Receita</>}
        </button>
      </form>
    </div>
  );
